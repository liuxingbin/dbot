...
def get_args():
    ...
    parser.add_argument('--model_type', type=str,
                        default="beit", help="model toye for clip or beit")
...
def main(args, ds_init):
    ...
    patch_size = (model.net.visual.patch_size,
                  model.net.visual.patch_size) if args.model_type == 'clip' else model.patch_embed.patch_size
    print("Patch size = %s" % str(patch_size))
    args.window_size = (args.input_size //
                        patch_size[0], args.input_size // patch_size[1])
    args.patch_size = patch_size

    if args.finetune and args.model_type != "clip":
    ...
    num_layers = model.net.visual.layers if args.model_type == "clip" else model_without_ddp.get_num_layers()
    ...
    if args.resume and (os.path.exists(args.resume) or args.enable_deepspeed):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.output_dir and args.save_ckpt:
            if (epoch + 1) % args.save_ckpt_freq == 0:
                utils.save_model(
                    args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
                    loss_scaler=loss_scaler, epoch=epoch+1, model_ema=model_ema)
