...
from tokenizer import CLIP, DINO, SELF
...
class MetricLogger(object):
    ...
    def log_every(self, iterable, print_freq, header=None, dataset_len=None):
        i = 0
        if not header:
            header = ''
        if not dataset_len:
        ...
        for _ in range(dataset_len):
            ...
            if i % print_freq == 0 or i == dataset_len - 1:
                eta_seconds = iter_time.global_avg * (dataset_len - i)
                eta_string = str(datetime.timedelta(seconds=int(eta_seconds)))
                if torch.cuda.is_available():
                    print(log_msg.format(
                        i, dataset_len, eta=eta_string,
                        meters=str(self),
...
def multi_stage_cosine_scheduler(stage_epochs, base_value, final_value, epochs, niter_per_ep, warmup_epochs=0,
                     start_warmup_value=0, warmup_steps=-1):
    ema_schedule = []
    for index, i in enumerate(stage_epochs[1:]):
        temp = cosine_scheduler(base_value, final_value, i - stage_epochs[index], niter_per_ep, 
                                warmup_epochs=warmup_epochs, start_warmup_value=start_warmup_value,
                                warmup_steps=warmup_steps)
        ema_schedule = np.concatenate((ema_schedule, temp))
    return ema_schedule
...
def save_model(args, epoch, model, model_without_ddp, optimizer, loss_scaler, model_ema=None, optimizer_disc=None, save_ckpt_freq=1, last=False):
...
def auto_load_model(args, model, model_without_ddp, optimizer, loss_scaler, model_ema=None, optimizer_disc=None):
    ...
    if getattr(args, 'load_init', None) is not None: #false
    ...
    if not getattr(args, 'enable_deepspeed', False): #TRUE
        # torch.amp
...
def create_tok(weight_path, tokenizer_type, device, tokenizer_model, args=None, **kwargs):
    if tokenizer_type == "dino":
        print("use dino tokenizer")
        return DINO(weight_path, tokenizer_model, **kwargs).to(device)
    elif tokenizer_type == "clip":
        return CLIP(weight_path, tokenizer_model, **kwargs).to(device)
    elif tokenizer_type =='self':
        return SELF(
            weight_path, 
            tokenizer_model, 
