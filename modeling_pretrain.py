...
class VisionTransformerForMaskedImageModeling(nn.Module):
    ...
    def forward(self, x, bool_masked_pos, return_all_tokens=False, return_patch_tokens=False, discard_cls_token=True):
        if discard_cls_token:
            x = self.forward_features(
                x, bool_masked_pos=bool_masked_pos)  # 196 -> 197
            x = x[:, 1:]  # 197 -> 196
        else:
            x = self.forward_features(
                x, bool_masked_pos=bool_masked_pos[:, 1:])  # 196 -> 197
    ...
    def forward_intermediate(self, x, bool_masked_pos=None, layer_id=12, discard_cls_token=True):
        if bool_masked_pos is None:
            bool_masked_pos = torch.zeros(
                (x.shape[0], self.patch_embed.num_patches), dtype=torch.bool).to(x.device)
        x = self.patch_embed(x, bool_masked_pos=bool_masked_pos)
        batch_size, seq_len, _ = x.size()
        ...
        if isinstance(layer_id, list):
            ...
            for l, blk in enumerate(self.blocks):
                x = blk(x, rel_pos_bias=rel_pos_bias)
                if l in layer_id:
                    output_list.append(x[:, 1:]if discard_cls_token else x)
            return output_list
            ...
            for l, blk in enumerate(self.blocks):
                if l < layer_id:
                    ...
                    break
            return x[:, 1:]if discard_cls_token else x
        else:
...
def beit_base_patch16_224(**kwargs):
    return beit_base_patch16_224_8k_vocab(**kwargs)


@register_model
def beit_large_patch16_224(**kwargs):
    return beit_large_patch16_224_8k_vocab(**kwargs)


@register_model
def beit_huge_patch14_224(**kwargs):
    return beit_huge_patch14_224_8k_vocab(**kwargs)
