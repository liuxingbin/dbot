...
def get_args_parser():
    ...
    # Dataset parameters
    parser.add_argument('--datasource', default='ImageNet', choices=['CIFAR', 'CIFAR10', 'ImageNet', 'ImageNetLMDB', 'INAT', 
                        'INAT19', 'Flwrs', 'Cars'], type=str, help='data source')
    parser.add_argument('--data', default='data/imagenet', type=str, help='path to dataset')
    parser.add_argument('--datalist', type=str, default=None, help='data list')
    parser.add_argument('--vdatalist', type=str, default=None, help='val data list')
...
def build_dataset(is_train, args):
    ...
    if args.datasource == 'CIFAR':
        dataset = datasets.CIFAR100(args.data, train=is_train, transform=transform)
        nb_classes = 100
    elif args.datasource == 'CIFAR10':
        dataset = datasets.CIFAR10(args.data, train=is_train, transform=transform)
        nb_classes = 10
    elif args.datasource == 'ImageNet':
        root = os.path.join(args.data, 'train' if is_train else 'val')
        dataset = datasets.ImageFolder(root, transform=transform)
        nb_classes = 1000
    elif args.datasource == 'INAT':
        dataset = INatDataset(args.data, train=is_train, year=2018,
                              category=args.inat_category, transform=transform)
        nb_classes = dataset.nb_classes
    elif args.datasource == 'INAT19':
        dataset = INatDataset(args.data, train=is_train, year=2019,
                              category=args.inat_category, transform=transform)
        nb_classes = dataset.nb_classes
    elif args.datasource == 'Cars':
        dataset = CarsDataset(args.data, train=is_train, transform=transform)
        nb_classes = dataset.nb_classes
    else:
        dataset = FlwrsDataset(args.data, train=is_train, transform=transform)
        nb_classes = dataset.nb_classes
